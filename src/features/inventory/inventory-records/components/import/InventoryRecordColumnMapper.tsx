...
export default function InventoryRecordColumnMapper({
  ...
  function autoMap() {
    ...
    excelColumns.forEach((excelCol) => {
      const matched = systemColumns.find(
        (col) =>
          normalize(col.label) === normalize(excelCol) &&
          !used.has(col.field_key),
      );
  ...
  return (
    <div className="rounded-xl border border-gray-200 shadow-sm p-4 space-y-4">
